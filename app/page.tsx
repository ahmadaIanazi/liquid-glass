...
import { TopNavigationBar } from "@/components/ActionBar";
import { BottomNavbar } from "@/components/DynamicFloatingButton";
import { BottomSheet } from "@/components/BottomSheet";
import { LiquidGlassFilters } from "@/components/LiquidGlassFilters";
import { useLiquidGlassMultiSensor } from "@/hooks/use-liquid-glass-multi-sensor";
import { Eye, Star, Heart, Zap, Music, Camera, Coffee, Book } from "lucide-react";
import Image from "next/image";

export default function HomePage() {
  // Use the multi-sensor liquid glass detector
  const { sensors, getSensorById } = useLiquidGlassMultiSensor({
    sensors: [
      {
        id: "top-nav",
  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50'>
      ...
      {/* Your liquid glass element - positioned in center */}
      <div className='fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 liquid-glass-wrapper rounded-2xl'>
        ...
        <div className={`liquid-glass-content ${centerElementSensor?.isDark ? "dark" : ""} px-10 py-2 rounded-2xl`}>
          <span className='text-2xl transition-colors duration-200'>TEST</span>
          <Eye className='w-4 h-4 transition-colors duration-200' />
          <span
            className='text-2xl transition-colors duration-200'
            style={{
              color: centerElementSensor?.isDark ? "#ef4444" : "#dc2626", // Adaptive red color
            }}
          >
            TEST2
          </span>
