...
interface LiquidGlassDetectorBottomOptions {
  threshold?: number;
  debug?: boolean;
  sensorSize?: { width: number | string; height: number | string };
}

interface LiquidGlassDetectorBottomResult {
  isDark: boolean;
  brightness: number;
  isLight: boolean;
...
export function useLiquidGlassDetectorBottom(options: LiquidGlassDetectorBottomOptions = {}): LiquidGlassDetectorBottomResult {
  const [brightness, setBrightness] = useState(0.5);
  const sensorRef = useRef<HTMLDivElement>(null);

  const { threshold = 0.75, debug = false, sensorSize = { width: 200, height: 100 } } = options;

  const analyzeBackground = useCallback(async () => {
    if (!sensorRef.current || typeof window === "undefined") {
      if (debug) console.log("❌ Bottom sensor ref not available");
      return;
    }
    ...
    try {
      ...
      if (rect.width === 0 || rect.height === 0) {
        if (debug) console.log("❌ Bottom sensor not visible");
        return;
      }
      ...
      if (validSamples > 0) {
        ...
        if (debug) {
          console.log("📊 Bottom Pixel Analysis complete:", {
        ...
        if (debug) console.log("🔄 No valid pixels found in bottom sensor, using fallback");
        setBrightness(0.5); // Fallback
      }
    } catch (error) {
      console.warn("Liquid glass bottom background detection failed:", error);
  ...
  const SensorComponent: React.ComponentType<{ className?: string }> = ({ className = "" }) => (
    <div
      ...
      style={{
        ...
        // backgroundColor: debug ? "rgba(255, 12, 0, 0.4)" : "transparent",
